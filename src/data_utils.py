...
num_threads = 24
pandarallel.initialize(nb_workers=num_threads, progress_bar=True)
...
def process_cdc_loc():
    locs = pd.read_csv(url_cdc_locations, dtype={'location': str})
    csse_deaths = pd.read_csv(url_csse_us_deaths)
...
def process_cdc_truth_from_csse(target, stat_type='inc'):
    if target == 'confirmed':
        daily_ts = pd.read_csv(url_csse_us_confirmed)
    elif target == 'deaths':
        daily_ts = pd.read_csv(url_csse_us_deaths)
    else:
...
def load_new_data(data_fp, config, logger=print):
    ...
    if used_dates[-1] >= required_fea_date:
        ...
        # then we suplement enough dates
